...
QUESTIONS = [
    ...
    question(
        ...
        False,
        "电梯旁边的一楼到二层楼梯间",
        "睁大眼睛，在电梯旁边的一楼到二楼楼梯间，所有固定贴牌中找出被大写的字母。找出缺失的字母和它们在字母表中的位置，得到 8 个数字。再回到这段毕业小景中，提取这8个数字对应的字：\n\n「一群同学在草地大笑，义工小辉举着闪光相机拍下今日毕业新照。风把学士帽吹得东倒西歪，大家一边追帽子一边嚷着“再来一张”，好像毕业不是散场，而是热热闹闹地换个地方继续出发。」",
    ...
    question(
        ...
        False,
        "一层与二层门牌",
        "门牌间也有彼此的秘密。请先在一层房间中，找到门牌名字为 7 个汉字的房间，记下它对应的房间号 abc。再去寻找门牌号为 b-a a c 的三位数字房间，这个房间门口同侧挂着一幅书法作品，书法内容即为答案。",
    ...
    with connect(db_path) as con:
        ...
        if not row["cyber_gift_at"]:
            ...
            create_participant_event(
                ...
                {
                    "title": "赛博礼物已送达",
                    "message": "谢谢你一起完成这趟楼里的旅程。愿今晚的烟花，也算一份来自社系毕业晚会的祝福。",
                },
                current_time,
            )
        row = con.execute("SELECT * FROM redeem_codes WHERE code = ?", (code,)).fetchone()
